...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    # Your code here
    # raise NotImplementedError
    z = 1 - (np.matmul(feature_matrix,theta) + theta_0) * labels
    z = np.where(z < 0, 0, z)
...
def perceptron_single_step_update(
    ...
    if z < 0.0001: # 0.0001 is used in order to mitigate numerical instability
...
def average_perceptron(feature_matrix, labels, T):
    ...
    theta_sum_update = theta
    theta_0_sum_update = theta_0
    counter = 0
    for t in range(T):
        for i in get_order(nsamples):
            theta, theta_0 = perceptron_single_step_update(feature_matrix[i, :],
    ...
    theta_final = theta_sum_update/counter
    theta_0_final = theta_0_sum_update/counter
...
def pegasos(feature_matrix, labels, T, L):
    ...
    theta_0 = 0
    nsamples = feature_matrix.shape[0]
    num_of_progression = 1
    for t in range(T):
        for i in get_order(nsamples):
            eta = 1/(num_of_progression**0.5)
...
def classify(feature_matrix, theta, theta_0):
    ...
    # Your code here
    # raise NotImplementedError
    z = np.matmul(feature_matrix, theta) + theta_0
    result = np.where(z > 0.00001, 1, -1)  # 0.00001 in order to avoid numeric instabilities
    return result
...
def classifier_accuracy(
    ...
    train_labels_pred = classify(train_feature_matrix, theta, theta_0)
    train_accuracy = accuracy(train_labels_pred, train_labels)
    val_labels_pred = classify(val_feature_matrix, theta, theta_0)
    val_accuracy = accuracy(val_labels_pred, val_labels)
